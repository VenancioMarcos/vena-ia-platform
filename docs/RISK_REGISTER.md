...
| R-032 | MÉDIO | Métricas e tracing existem somente no processo, sem retenção ou backend operacional externo. | Package 2 adiciona contratos, cardinalidade fechada e endpoint admin opt-in; selecionar backend/exportação somente em gate posterior autorizado. | MITIGADO PARCIALMENTE v1.4 PACKAGE 2 / MONITORAR |
...
| R-010 | MÉDIO | Eventos sensíveis precisavam de correlação persistente com requests e mutações operacionais. | Packages 1–2 adicionam schema/redaction, request/correlation ID persistidos e auditoria de mutações allowlisted; validar retenção e operação integral no Package 3. | MITIGADO PARCIALMENTE v1.4 PACKAGE 2 / MONITORAR |
