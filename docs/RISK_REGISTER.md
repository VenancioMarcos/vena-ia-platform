...
| R-049 | CRÍTICO | Evidência de simulação pode ser interpretada como validação física, de máquina ou liberação produtiva. | Planning verification v1 cobre somente coerência/coverage/resources/precedence/setup geométrico preliminar, com `physical_validation=false`; material removal, collision e kinematics seguem gates v2.3. | MITIGADO PARCIALMENTE v2.2 PACKAGE 2 / GATE v2.3 |
