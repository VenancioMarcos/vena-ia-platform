...
| R-049 | CRÍTICO | Evidência de simulação pode ser interpretada como validação física, de máquina ou liberação produtiva. | Geometry evidence v1 explicita limitações, engine/corpus, revisão humana e ausência de output executável; simulation/planning da Package 2 ainda precisa provar separação de validação real. | ABERTO / GATE v2.2 PACKAGE 2 |
