...
| R-053 | ALTO | Uma interface que oculte evidências intermediárias ou permita download sem confirmação pode induzir uso do candidato fora do contexto controlado. | TASK-V31-006 expõe Manufacturing Geometry, Process Plan, Toolpath, Level-2 e provenance do Digital Thread; o download exige confirmação explícita de NON_PRODUCTION/revisão humana/ausência de autoridade física. A confirmação é somente UX e não altera a validação server-side. | MITIGADO PARCIALMENTE v3.1 / MONITORAR |
