...
| R-044 | ALTO | A integração CAD→feature→engineering→planning→CNC neutro→report pode transformar evidência parcial de um domínio em conclusão indevida de outro. | Contrato agregado deve preservar versões, assumptions, limitations, uncertainty, status parcial e traceability por elo; nenhuma ausência pode virar sucesso. | ABERTO / GATE v2.0 PACKAGE 1 |
| R-045 | CRÍTICO | Um workflow integrado e dashboard podem ser interpretados como sistema aprovado para produção ou máquina. | Marcação não produtiva, revisão humana, `SIMULATION_ONLY`, `executable_output=false` e proibições de toolpath/G/M-code/NC/DNC/transmissão/controle em contrato, UI, testes e documentação. | ABERTO / GATE v2.0 PACKAGES 1–3 |
