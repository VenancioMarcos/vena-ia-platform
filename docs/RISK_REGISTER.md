...
| R-048 | CRÍTICO | Catálogos e dados Engineering globais podem vazar ou ser alterados entre organizações quando a plataforma ampliar automação. | Package 1 adiciona ownership por Organization, legado bloqueado, referências read-only, migration reversível, autorização fail-closed e testes cross-org sem confiar em body/header. Lifecycle/export/retention do Package 2 ainda são gates. | MITIGADO PARCIALMENTE v2.1 PACKAGE 1 / MONITORAR |
| R-049 | CRÍTICO | Evidência de simulação pode ser interpretada como validação física, de máquina ou liberação produtiva. | v2.2 deve separar modelo/evidence de validação real, registrar assumptions/envelope/engine/corpus e manter `NON_PRODUCTION`, revisão humana e ausência de output executável. | ABERTO / GATE v2.2 |
