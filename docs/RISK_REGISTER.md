...
| R-044 | ALTO | A integração CAD→feature→engineering→planning→CNC neutro→report pode transformar evidência parcial de um domínio em conclusão indevida de outro. | O aggregate versionado preserva schema/rule/catalog/source refs, estados fechados, missing inputs, limitations e revisão humana; testes provam que feature, recommendation, planning e CNC não elevam o estado upstream. | MITIGADO PARCIALMENTE v2.0 PACKAGE 1 / MONITORAR |
| R-045 | CRÍTICO | Um workflow integrado e dashboard podem ser interpretados como sistema aprovado para produção ou máquina. | Package 1 marca `NON_PRODUCTION`, revisão humana, `simulation_only=true`, `executable_output=false` e proíbe toolpath/G/M-code/NC/DNC/transmissão/controle. Dashboard/UX do Package 3 ainda é gate. | ABERTO / GATE v2.0 PACKAGES 1–3 |
| R-046 | ALTO | Assistente/agente pode sobrescrever regra determinística, preencher lacuna ou elevar sugestão a decisão. | Agentes somente leem contratos allowlisted, não alteram resultados, devem citar evidence e falhar fechado; aprovação humana permanece externa. | ABERTO / GATE v2.0 PACKAGE 2 |
| R-047 | ALTO | Síntese Research integrada pode extrapolar fontes, DOE preliminar ou ANOVA descritiva como validação científica. | Grounding documento/página/chunk, limitações explícitas, ausência como `NOT_AVAILABLE`, revisão científica humana e proibição de inferência autônoma. | ABERTO / GATE v2.0 PACKAGE 2 |
