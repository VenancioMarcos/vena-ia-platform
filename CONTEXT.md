...
* **v1.2 Security Package 2:** a mesma Draft PR #13 passa a invalidar no processo o token apresentado no logout, rejeita reutilização posterior e tokens emitidos no futuro, preserva logout idempotente e melhora a mensagem frontend para `429`. A denylist não é compartilhada entre réplicas nem persiste em reinício; revogação distribuída continua pendente antes de produção horizontal.
